...
from kar import events_create_subscription
from kar import events_cancel_subscription, events_get_subscription
...
# class.
class TestActorEvents(KarActor):
    ...
    def naming_event(self, event):
        ...
        if "suffix" in event:
            suffix = event["suffix"]
        self.set_name(event["name"], surname=surname, suffix=suffix)
        assert self.get_name() == "John Doe"

    async def subscribe(self, topic):
        return await events_create_subscription(self, 'naming_event', topic)

    async def get_subscription(self):
        return await events_get_subscription(self)

    async def cancel_subscription(self):
        return await events_cancel_subscription(self)


if __name__ == '__main__':
    # Register actor type with the KAR runtime.
    app = actor_runtime([TestActorEvents], actor_server=app)
