...
async function warmUp () {
  for (let i = 0; i < WARM_UP_TXNS / CONCURRENCY; i++) {
    ...
    for (let j = 0; j < CONCURRENCY; j++) {
      ...
      if (r < 44) { promises.push(newOrderTxn(true)) } else if (r < 88) { promises.push(paymentTxn(true)) } else if (r < 92) { promises.push(orderStatusTxn(true)) } else if (r < 96) { promises.push(deliveryTxn(true)) } else { promises.push(stockLevelTxn(true)) }
    }
...
async function newOrderTxn (isWarmUp = false) {
  ...
  const txnId = uuidv4()
  const txnActor = actor.proxy('NewOrderTxn', txnId)
  if (!isWarmUp) {
...
async function getLatency (totalTime) {
  let totalLatency = 0; let totalCnt = 0
...
async function sendTxns () {
  for (let i = 0; i < NUM_TXNS / CONCURRENCY; i++) {
    const r = getRandomInt(1, 100)
    if (r < 44) { await newOrderTxn(); txnMetadata.newOrder.cnt++ } else if (r < 88) { await paymentTxn(); txnMetadata.payment.cnt++ } else if (r < 92) { await orderStatusTxn(); txnMetadata.orderStatus.cnt++ } else if (r < 96) { await deliveryTxn(); txnMetadata.delivery.cnt++ } else { await stockLevelTxn(); txnMetadata.stockLevel.cnt++ }
  }
}
