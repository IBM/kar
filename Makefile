...
docker:
	...
	docker build -f examples/docker/Dockerfile_js -t $(DOCKER_IMAGE_PREFIX)examples-js:$(DOCKER_IMAGE_TAG) --build-arg SDK_BASE=$(KAR_JS_SDK) examples
...
dockerPush: docker
	...
	docker push $(DOCKER_IMAGE_PREFIX)examples-js:$(DOCKER_IMAGE_TAG)
...
kindPush: docker
	...
	kind load docker-image $(DOCKER_IMAGE_PREFIX)examples-js:$(DOCKER_IMAGE_TAG)
